...
def create_relational_record(collection: str, values: dict[str, Any]) -> dict[str, Any]:
    ...
    defaults = {"tasks": {"status": "open", "priority": "normal"}, "rfis": {"status": "open", "priority": "Medium"}, "approvals": {"status": "pending"}, "cost_control": {"status": "draft", "amount": 0}}[collection]
    for key, value in defaults.items(): clean.setdefault(key, value)
    columns = list(clean)
...
def update_relational_record(collection: str, record_id: Any, values: dict[str, Any]) -> dict[str, Any] | None:
    table, _, select = _config(collection)
    clean = _clean(collection, values)
    if not clean: raise ValueError(f"No supported {collection} changes supplied.")
