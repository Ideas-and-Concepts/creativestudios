...
def _inject_dashboard_css() -> None:
    st.markdown(
        ...
        .cs-dash-appbar-logo{height:34px;width:34px;object-fit:contain}.cs-dash-icon{font-size:16px;color:#334155}.cs-dash-user{width:28px;height:28px;border:1px solid #dbe2ea;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#475569;margin-left:7px}
...
def render_dashboard(database: dict[str, Any]) -> None:
    ...
    # Compact top application bar matching the Vercel shell.
    ...
    with left:
        ...
        if projects:
            ...
            fig = px.line(pd.concat(frames, ignore_index=True), x="Date", y="Progress", color="Project", markers=True, range_y=[0, 100])
