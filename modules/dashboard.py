...
The dashboard reads canonical relational records whenever Neon is configured,
including operational project-control modules. Local JSON remains the fallback.
"""
...
def _records(database: dict[str, Any], key: str) -> list[dict[str, Any]]:
    ...
    if database_backend() == "neon":
        relational = {
            ...
            "tasks": lambda: get_relational_records("tasks"),
            "rfis": lambda: get_relational_records("rfis"),
            "approvals": lambda: get_relational_records("approvals"),
            "cost_control": lambda: get_relational_records("cost_control"),
            "construction_activities": lambda: get_relational_records("construction"),
        }.get(key)
        if relational:
            return relational()
...
def render_dashboard(database: dict[str, Any]) -> None:
    ...
    if project_id:
        documents = _scope(documents, project_id); drawings = _scope(drawings, project_id); boq = _scope(boq, project_id); procurement = _scope(procurement, project_id)
        tasks = _scope(tasks, project_id); rfis = _scope(rfis, project_id); construction = _scope(construction, project_id); engineering = _scope(engineering, project_id); mep = _scope(mep, project_id)
    ...
    procurement_value = sum(_number(r.get("total_amount"), _number(r.get("total"), _number(r.get("grand_total"), _number(r.get("amount"))))) for r in procurement)
    active_tasks = sum(_status(r) in {"open", "in_progress", "blocked", "review"} for r in tasks)
    completed_tasks = sum(_status(r) in {"completed", "closed"} for r in tasks)
    open_rfis = sum(_status(r) not in {"closed", "completed", "cancelled"} for r in rfis)
    ...
    with left:
        st.markdown('<div class="cs-dash-panel">', unsafe_allow_html=True); _panel("Portfolio status", "Project mix within the selected scope")
        counts = {}
        for project in project_scope:
            state = _status(project, "planning"); counts[state] = counts.get(state, 0) + 1
        ...
        if ordered:
            ...
            fig = px.bar(frame, x="Status", y="Projects", text="Projects"); fig.update_traces(textposition="outside"); fig.update_layout(height=245, margin=dict(l=0,r=0,t=8,b=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, font=dict(size=9)); fig.update_xaxes(showgrid=False, title=None); fig.update_yaxes(showgrid=True, title=None, dtick=1); st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
        else: st.markdown('<div class="cs-empty">Create projects to populate the portfolio view.</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with right:
        st.markdown('<div class="cs-dash-panel">', unsafe_allow_html=True); _panel("Workstream progress", "Recorded progress, never inferred from missing data")
        ...
        source = "Neon PostgreSQL" if database_backend() == "neon" else "Local JSON"; st.markdown(f'<div class="cs-insight"><strong>Source:</strong> {escape(source)}. Operational tasks, RFIs, approvals, cost control and construction now read from canonical relational tables when Neon is active.</div>', unsafe_allow_html=True); st.markdown('</div>', unsafe_allow_html=True)

    left2, right2 = st.columns(2)
    with left2:
        st.markdown('<div class="cs-dash-panel">', unsafe_allow_html=True); _panel("Documentation control", "Documents and drawing register health")
        frame = pd.DataFrame({"Metric": ["Documents", "Approved documents", "Drawings"], "Count": [len(documents), approved_documents, len(drawings)]}); fig = px.bar(frame, x="Metric", y="Count", text="Count"); fig.update_traces(textposition="outside"); fig.update_layout(height=230, margin=dict(l=0,r=0,t=8,b=0), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, font=dict(size=9)); fig.update_xaxes(showgrid=False, title=None); fig.update_yaxes(showgrid=True, title=None); st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False}); st.markdown('</div>', unsafe_allow_html=True)
    with right2:
        st.markdown('<div class="cs-dash-panel">', unsafe_allow_html=True); _panel("Commercial snapshot", "BOQ and procurement values from the active project scope")
