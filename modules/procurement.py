...
from modules.project_context import project_label, project_options, select_project
...
def suppliers(db): return get_relational_suppliers() if database_backend() == "neon" else get_records("suppliers", db)
        ...
        if not active: st.info("Add an active supplier before creating a purchase order.")
            ...
            if submit:
                try:
                    ...
                    supplier=active[supplier_labels.index(supplier_choice)]; items=[]
                    for link,description,quantity,unit,rate in item_forms:
                        boq_id=dict(boq_options)[link]
                        if boq_id and not description.strip():
                            boq=next((b for b in boq_rows if str(b.get("id"))==str(boq_id)),{}); description=str(boq.get("description", "")); unit=str(boq.get("unit",unit)); rate=float(boq.get("rate",rate) or rate)
                    ...
                    subtotal=sum(money(i["quantity"])*money(i["unit_rate"]) for i in items); tax=(subtotal*money(tax_rate)/Decimal("100")).quantize(Decimal("0.01")); values={"project_id":str(project_id),"supplier_id":str(supplier["id"]),"po_number":po_number.strip(),"status":po_status,"order_date":datetime.combine(order_date,datetime.min.time()),"expected_delivery_date":datetime.combine(expected,datetime.min.time()) if expected else None,"tax_amount":str(tax),"notes":notes.strip()}
                    if database_backend()=="neon": create_relational_purchase_order(values,items)
                    else:
                        next_id=max([int(x.get("id",0)) for x in database.get("procurement",[]) if str(x.get("id","")).isdigit()] or [0])+1; database.setdefault("procurement",[]).append({"id":next_id,"project_id":project_id,"po_number":po_number.strip(),"supplier":supplier.get("name",""),"status":po_status,"quantity":sum(float(i["quantity"]) for i in items),"unit_price":float(subtotal),"amount":float(subtotal+tax),"expected_delivery":expected.isoformat() if expected else "","notes":notes.strip(),"created_at":datetime.now().isoformat(timespec="seconds")}); save_memory(database)
                    st.success("Purchase order created."); st.rerun()
                except Exception as exc: st.error(str(exc))
        if order_rows:
            choices=[f"{o.get('po_number','')} | {o.get('supplier_name',o.get('supplier',''))}" for o in order_rows]; selected_po=st.selectbox("Purchase order actions",choices,key="po_action"); order=order_rows[choices.index(selected_po)]
