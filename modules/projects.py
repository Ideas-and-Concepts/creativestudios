            ...
            try:
                project = add_record(
                    "projects",
                    {
                        ...
                        "status": status,
                        "estimated_budget": round(float(budget), 2),
                        "notes": notes.strip(),
                        "created_at": datetime.now().isoformat(timespec="seconds"),
                    },
                    database,
                )
                st.success(f"Project {project['id']} created successfully.")
                st.rerun()
            except Exception as exc:
                st.error("Unable to save the project.")
                    ...
                    try:
                        update_record(
                            ...
                            project_id,
                            {
                                ...
                                "estimated_budget": round(float(edited_budget), 2),
                        ...
                        st.error("Unable to update the project.")
                        with st.expander("Technical details"):
                            st.exception(exc)
