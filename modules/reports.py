...
COLLECTIONS = [
    ...
    "construction", "tasks", "rfis", "approvals", "cost_control",
]
...
def _cost_total(database: dict[str, Any], cost_type: str | None = None) -> float:
    ...
    return sum(
        ...
        if cost_type is None or record.get("cost_type") == cost_type
    )
...
def render_reports_module(database: dict[str, Any]) -> None:
    ...
    c4.metric("Open RFIs", sum(r.get("status") in {"Open", "Under Review"} for r in rfis))

    st.subheader("Delivery Controls")
    status_rows = [
        {
            "Project Status": status.replace("_", " ").title(),
            "Projects": sum(str(project.get("status", "")) == status for project in projects),
        }
