...
DEFAULT_DATABASE: dict[str, Any] = {
    ...
    "architecture": [], "engineering": [], "mep": [], "boq": [], "construction": [],
    "rfis": [], "tasks": [], "teams": [], "site_logs": [],
...
def load_memory() -> dict[str, Any]:
    try:
        ...
        if not DB_FILE.exists():
            database = copy.deepcopy(DEFAULT_DATABASE); save_memory(database); return database
...
def save_memory(db: dict[str, Any]) -> bool:
    database = _normalize_database(db); DB_FILE.parent.mkdir(parents=True, exist_ok=True)
    temporary_path = None
    try:
        with tempfile.NamedTemporaryFile(mode="w", encoding="utf-8", suffix=".tmp", prefix="creativestudios_", dir=DB_FILE.parent, delete=False) as temporary:
            json.dump(database, temporary, indent=2, ensure_ascii=False, default=_json_default); temporary.flush(); os.fsync(temporary.fileno()); temporary_path = Path(temporary.name)
...
def get_records(collection: str, db: dict[str, Any]) -> list[dict[str, Any]]:
    return [r for r in _ensure_collection(db, collection) if isinstance(r, dict)]
...
def get_record(collection: str, record_id: Any, db: dict[str, Any]) -> dict[str, Any] | None:
    for record in _ensure_collection(db, collection):
        if isinstance(record, dict) and str(record.get("id")) == str(record_id): return record
    return None
...
def update_record(collection: str, record_id: Any, updates: dict[str, Any], db: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for index, record in enumerate(records):
        if isinstance(record, dict) and str(record.get("id")) == str(record_id):
            ...
            if not save_memory(db): records[index] = original; raise IOError("Unable to save the database.")
            return updated
    return None
...
def delete_record(collection: str, record_id: Any, db: dict[str, Any]) -> bool:
    ...
    for index, record in enumerate(records):
        if isinstance(record, dict) and str(record.get("id")) == str(record_id):
            ...
            if not save_memory(db): records.insert(index, deleted); raise IOError("Unable to save the database.")
            return True
    return False
