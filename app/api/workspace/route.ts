...
const schemas = {
  documents: z.object({
    ...
    discipline: z.string().trim().max(100).optional().nullable(),
    status: documentStatus.optional(),
    fileName: z.string().trim().max(255).optional().nullable(),
    fileUrl: z.string().trim().max(1000).optional().nullable(),
    revision: z.string().trim().max(30).optional().nullable(),
...
function normalizeValues(module: ModuleName, values: Record<string, unknown>) {
  ...
  if (module === "documents") { normalized.status ??= "Draft"; normalized.revision ??= "1"; normalized.isApproved ??= normalized.status === "Approved"; }
  return normalized;
}

export async function GET(request: Request) {
  const module = moduleName(request); if (!module) return NextResponse.json({ error: "Unsupported workspace module." }, { status: 400 });
  ...
  try {
    ...
    const rows = projectId ? await getDb().select().from(table).where(eq(table.projectId, projectId)).orderBy(desc(table.createdAt)) : await getDb().select().from(table).orderBy(desc(table.createdAt));
    return NextResponse.json({ data: rows });
  } catch (error) { console.error(`GET /api/workspace?module=${module} failed`, error); return NextResponse.json({ error: "Database is not configured or unavailable." }, { status: 503 }); }
}

export async function POST(request: Request) {
  const module = moduleName(request); if (!module) return NextResponse.json({ error: "Unsupported workspace module." }, { status: 400 });
  const parsed = await parseBody(request, module); if (!parsed.success) return NextResponse.json({ error: "Invalid record data.", issues: parsed.error.flatten() }, { status: 400 });
  try {
    const table = tables[module] as any; const values = normalizeValues(module, parsed.data as Record<string, unknown>);
    const result = await getDb().insert(table).values(values).returning();
    return NextResponse.json({ data: (result as any[])[0] }, { status: 201 });
  } catch (error) { console.error(`POST /api/workspace?module=${module} failed`, error); return NextResponse.json({ error: "Unable to create record." }, { status: 500 }); }
}

export async function PATCH(request: Request) {
  const module = moduleName(request); const id = new URL(request.url).searchParams.get("id");
  ...
  try {
    ...
    const row = (result as any[])[0]; if (!row) return NextResponse.json({ error: "Record not found." }, { status: 404 });
    return NextResponse.json({ data: row });
  } catch (error) { console.error(`PATCH /api/workspace?module=${module}&id=${id} failed`, error); return NextResponse.json({ error: "Unable to update record." }, { status: 500 }); }
}
...
export async function DELETE(request: Request) {
  ...
  if (!validId(id)) return NextResponse.json({ error: "Invalid record id." }, { status: 400 });
  try {
    const table = tables[module] as any; const result = await getDb().delete(table).where(eq(table.id, id)).returning({ id: table.id }); const row = (result as any[])[0];
    if (!row) return NextResponse.json({ error: "Record not found." }, { status: 404 });
    return NextResponse.json({ data: row });
  } catch (error) { console.error(`DELETE /api/workspace?module=${module}&id=${id} failed`, error); return NextResponse.json({ error: "Unable to delete record." }, { status: 500 }); }
}
