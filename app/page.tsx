...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try { const health = await fetch("/api/health", { cache: "no-store" }); const data = await health.json(); setDatabaseReady(Boolean(data.ok && data.database)); } catch { setDatabaseReady(false); }
      try { const response = await fetch("/api/dashboard/summary", { cache: "no-store" }); const data = await response.json(); if (response.ok && data.data) setSummary(data.data); } catch { setSummary({ projects: 0, drawings: 0, boqItems: 0, activeWorks: 0 }); }
    };
    void refresh();
  }, []);
  ...
  const changeTheme = () => { const next = theme === "dark" ? "light" : "dark"; setTheme(next); window.localStorage.setItem(THEME_KEY, next); };
  ...
  const savePageConfig = () => { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(Object.fromEntries(modules.map((item) => [item.href, { name: item.name, description: item.description }])))); setEditingPages(false); };
  ...
  return <main className={theme === "light" ? "app light" : "app"}>
    <aside className="sidebar">
      ...
      <div className="divider" />
      <nav className="nav" aria-label="Workspace navigation">{grouped.map(({ group, items }) => <div key={group}><div className="section-label">{group}</div>{items.map((item) => <Link key={item.href} href={item.href} className={item.name === active ? "nav-item active" : "nav-item"} onClick={() => setActive(item.name)}>{item.name}</Link>)}</div>)}</nav>
