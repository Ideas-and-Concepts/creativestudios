...
function NavigationContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <nav className="cs-drawer-nav">
    ...
    <div className="cs-drawer-group"><div className="cs-drawer-label">System</div><Link href="/settings" onClick={onNavigate} className={`cs-drawer-link${isActive("/settings") ? " active" : ""}`}><Settings aria-hidden="true" />Settings</Link></div>
  </nav>;
}

function DrawerHeader({ close }: { close?: () => void }) {
  return <div className="cs-drawer-head">
    ...
    {close && <button className="cs-drawer-close" aria-label="Close menu" onClick={close}><X /></button>}
  </div>;
}
...
export default function AecNavigation() {
  ...
  return <>
    <style>{`
      ...
      .cs-menu-button{position:fixed;z-index:1600;top:21px;left:82px;width:38px;height:38px;border:1px solid #e2e8f0;border-radius:8px;background:#fff;color:#334155;display:grid;place-items:center;cursor:pointer;box-shadow:0 4px 14px rgba(15,23,42,.08)}
      ...
      .cs-drawer{position:fixed;z-index:1500;inset:0 auto 0 0;width:280px;max-width:86vw;height:100dvh;max-height:100dvh;background:#fff;border-right:1px solid #e5e7eb;box-shadow:18px 0 50px rgba(15,23,42,.18);display:flex;flex-direction:column;overflow:hidden}
      .cs-desktop-drawer{display:flex;z-index:1200;box-shadow:4px 0 22px rgba(15,23,42,.06);transform:none;animation:none}.cs-mobile-drawer{display:none;animation:csDrawerIn .16s ease-out}
    ...
    <div className="cs-shell-actions" aria-label="Workspace actions"><button className="cs-shell-action" aria-label="Notifications"><Bell /></button><span className="cs-avatar" aria-label="User profile">CS</span></div>
