  ...
  return (
    <>
      <style>{`
        ...
        .aec-menu{grid-column:1 / -1;display:flex;justify-content:center;align-items:center;gap:6px;flex-wrap:wrap;padding-bottom:2px}.aec-menu details{position:relative}.aec-menu summary{list-style:none;cursor:pointer;color:#475569;font-size:12px;font-weight:700;padding:8px 12px;border:1px solid transparent;border-radius:8px}.aec-menu summary::-webkit-details-marker{display:none}.aec-menu summary:hover,.aec-menu details[open] summary,.aec-menu details.has-active summary{color:#1d4ed8;background:#eff6ff;border-color:#bfdbfe}
        ...
        <div className="aec-nav-inner">
          ...
          <nav className="aec-menu" aria-label="AEC page navigation">
            {groups.map((group) => {
              ...
              return (
                <details key={group.name} className={active ? "has-active" : undefined}>
                  <summary>{group.name}</summary>
                  <div className="aec-dropdown">
                    {group.items.map((item) => {
                      const itemActive = pathname === item.href || pathname.startsWith(`${item.href}/`);
                      return <Link key={item.href} href={item.href} className={itemActive ? "active" : ""}>{item.label}</Link>;
                    })}
                  </div>
                </details>
              );
